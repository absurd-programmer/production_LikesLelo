...
export const updateProfileController = async (req, res) => {
  try {
    ...
    if (password && password.length < 6) {
      return res.json({
        error: "Password is required and it should be 6 characters long",
      });
    }
...
export const getAllOrdersController = async (req, res) => {
  try {
    ...
    console.log("Orders fetched:", orders.length); // Debugging line
    res.json(orders);
  } catch (error) {
    console.error("Error fetching orders:", error); // Enhanced logging
    res.status(500).send({
      success: false,
      message: "Error While Getting Orders",
      error: error.message,
